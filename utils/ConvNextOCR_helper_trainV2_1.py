...
def create_text_visualization(gt_text: str, pred_text: str, width: int = 800, height_per_text: int = 80) -> np.ndarray:
    """
...
def convert_to_rec_examples(
    ...
    if visual_context and visual_context.get("enabled") and samples:
        ...
        if limit > 0 and visual_context["count"] < limit:
            ...
            try:
                save_joint_visual(img_tensor, polys, samples, out_path, rgb=rgb, crop_max=visual_context.get("crop_max", 4))
...
def run_detection_epoch(state: dict, config: TrainerConfig, epoch: int, total_epochs: int):
    ...
    for batch_idx, (imgs, polys_batch) in enumerate(tqdm(loader, desc=f"Det Epoch {epoch}/{total_epochs}", leave=False)):
        ...
        if realtime_viz and batch_idx % viz_interval == 0 and batch_idx > 0:
            with torch.no_grad():
                # Get first image from batch for visualization
                sample_img = imgs[0].detach().cpu()
                sample_polys = polys_batch[0] if len(polys_batch) > 0 else []
                
                # Get predicted boxes from probability map
                # pred is a dict with keys: "shrink", "thresh", "shrink_logits", "thresh_logits", "binary"
                prob_map = pred["shrink"][0, 0].detach().cpu().numpy()  # First sample, first channel
                threshold = 0.3
                binary = (prob_map > threshold).astype(np.uint8)
                contours, _ = cv2.findContours(binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                pred_boxes = [cnt.squeeze() for cnt in contours if len(cnt) >= 4]
                ...
                cv2.waitKey(1)  # Non-blocking, 1ms wait
...
def run_recognition_epoch(state: dict, config: TrainerConfig, epoch: int, total_epochs: int):
    ...
    for batch_idx, (imgs, labels) in enumerate(tqdm(loader, desc=f"Rec Epoch {epoch}/{total_epochs}", leave=False)):
        ...
        with torch.cuda.amp.autocast(enabled=(device == "cuda")):
            ...
            if realtime_viz and batch_idx % viz_interval == 0 and batch_idx > 0 and preview:
                gt_text, pred_text = preview[0]  # Get first sample
