...
def sample_rec_crop(
    ...
    crop_tensor = torch.from_numpy(crop).unsqueeze(0)
    crop_tensor = F.interpolate(crop_tensor, size=(text_imgH, text_imgW), mode="bilinear", align_corners=False)
...
def convert_to_rec_examples(
    ...
    for poly, label in zip(polys, labels):
        s = sample_rec_crop(img_tensor, poly, label, text_imgH, text_imgW, rgb, margin_min_ratio, margin_ratio)
        if s is not None:
            samples.append(s)
...
class RecognitionCropDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if sample is None:
            resized = F.interpolate(img_tensor.unsqueeze(0), size=(self.text_imgH, self.text_imgW),
...
def run_detection_epoch(state: dict, config: TrainerConfig, epoch: int, total_epochs: int):
    ...
    for imgs, polys_batch in tqdm(loader, desc=f"Det Epoch {epoch}/{total_epochs}", leave=False):
...
def run_recognition_epoch(state: dict, config: TrainerConfig, epoch: int, total_epochs: int):
    ...
    for imgs, labels in tqdm(loader, desc=f"Rec Epoch {epoch}/{total_epochs}", leave=False):
        imgs = imgs.to(device, non_blocking=True)
