...
class BoxTextDataset(Dataset):
    """
    Combined JSON (per-box) expected format:
      [
        {"filename": "img1.png", "box": [[x,y],[x,y],[x,y],[x,y]], "label": ""},
        {"filename": "img1.png", "box": [[x,y],[x,y],[x,y],[x,y]], "label": ""},
        {"filename": "img2.png", "box": [[x,y],[x,y],[x,y],[x,y]], "label": ""}
      ]
    ...
    def __init__(
        ...
        if self.load_label or self.load_box_polygons:
            ...
            for ann in anns:
                ...
                grouped.setdefault(fname, {"labels": [], "boxes": []})
                grouped[fname]["labels"].append(lab)
                if self.load_box_polygons:
                    grouped[fname]["boxes"].append(box)
    ...
    def __getitem__(self, idx):
        if self.load_label or self.load_box_polygons:
            img_path, labels, boxes = self.samples[idx]
        else:
