...
from utils.ConvNextOCR_helper_trainV2_visualization import resize_and_pad_rec_tensor
...
_REC_TO_TENSOR = transforms.ToTensor()
_REC_NORMALIZE_RGB = transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))


def prepare_recognition_tensor(
    pil_img: Image.Image,
    target_h: int,
    target_w: int,
    preserve_aspect: bool,
) -> torch.Tensor:
    """Convert crop to normalized tensor with optional aspect-preserving padding."""
    tensor = _REC_TO_TENSOR(pil_img)
    tensor = resize_and_pad_rec_tensor(tensor, target_h, target_w, preserve_aspect)
    return _REC_NORMALIZE_RGB(tensor)
