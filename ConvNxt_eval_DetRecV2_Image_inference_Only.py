...
from utils.ConvNextOCR_helper_Eval import build_transform, db_postprocess, rescale_boxes


IMG_DIR = Path(r"D:\hvs\Hyvsion_Projects\OCR\dataset\projects\Dot_matrix_crop_251120\test")

REC_CHECKPOINT = Path(r"D:\hvs\Hyvsion_Projects\OCR\trainedModel\Text_Det_Rec\run_3\Rec\E1999_L0.000029_Rec.pth")
DET_CHECKPOINT = Path(r"D:\hvs\Hyvsion_Projects\OCR\trainedModel\Text_Det_Rec\run_3\Det\E347_L0.225306_Det.pth")

OUTPUT_CSV = Path(r"D:\hvs\Hyvsion_Projects\OCR\trainedModel\Text_Det_Rec\run_3\ImageOnly_inference_results.csv")

VISUALS_DIR = Path(r"D:\hvs\Hyvsion_Projects\OCR\trainedModel\Text_Det_Rec\run_3\ImageOnly_inference_visuals")
    ...
    if TEXT_BOXDETECTION_INFERENCE:
        ...
        for idx in tqdm(range(len(dataset)), desc="Detect+Rec", leave=False):
            ...
            for det_idx, (poly, score) in enumerate(detections):
                ...
                if crop is None:
                    continue
                tensor = rec_transform(crop.convert("RGB"))
        ...
        for idx in tqdm(range(len(dataset)), desc="Rec-only", leave=False):
            ...
            tensor = rec_transform(pil_img.convert("RGB"))
            rec_tensors.append(tensor)
